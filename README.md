...
2. `.env` ファイルを作成する（Dockerコマンドを実行する前に必ず行うこと）

> ⚠️ `.env` は Git 管理対象外のため、クローンしても自動では作成されません。各自ローカルで作成してください。
...
作成した `.env` ファイルに以下を記載する（値はメンバーに確認してください）:

```
SECRET_KEY_BASE=ここに発行されたキーを貼り付ける
```

3. 以下のコマンドを打ち込んでいく。(アプリのディレクトリ内にcdコマンドで移動してから)
